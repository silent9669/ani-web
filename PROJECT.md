...
|---|------|-------|-------------|--------|
| 1 | Installer Aesthetics & DMG Config | Generate a custom DMG background image incorporating the app's logo, place it in packaging/macos/, and configure tauri.conf.json. | None | PLANNED |
...
| 3 | Homebrew Tap CD Automation | Update release workflow to fix macOS Intel runner label to macos-13, download artifacts, and run generate-homebrew-cask.rb with local hashing support to update Casks/ani-desk.rb. Use gh CLI to set up repository secrets. | None | PLANNED |
| 4 | Cross-Platform Build & Verification | Verify Tauri build matrix outputs (macOS DMG, Windows MSI/EXE, Linux AppImage/DEB) and run local checks/tests. | M1, M2, M3 | PLANNED |
